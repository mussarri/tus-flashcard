...
export default function SessionSummaryScreen() {
  ...
  const { data: summary, isLoading } = useSessionSummary(
    sessionId,
    !!sessionId,
  );
          ...
          <View style={styles.cardTypeList}>
            {Object.entries(summary.cardsByType).map(([type, count]) => (
              <View key={type} style={styles.cardTypeRow}>
                ...
                <Text style={styles.cardTypeCount}>{String(count)}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        {summary.recommendedBacktrack.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={styles.recommendationList}>
              {summary.recommendedBacktrack.map(
                (
                  pattern:
                    | string
                    | number
