...
### 1.1 Directory Structure (Feature-First & Expo Router)
Since we use Expo Router, the app/ directory manages navigation, while the src/ directory manages business logic.

Plaintext
app/                    # Navigation (File-based Routing)
├── (auth)/             # Logical route grouping (e.g., login, register)
├── (tabs)/             # Tab-based navigation layout
└── _layout.tsx         # Root layout (Providers: Stripe, QueryClient, etc.)
src/
├── features/           # Feature-based modules (Domain-driven)
│   ├── Flashcards/
│   │   ├── components/ # UI specific to this feature
│   │   ├── hooks/      # feature-specific hooks (useFlashcards, etc.)
│   │   ├── store.ts    # Local Zustand store (if needed)
...
└── store/              # Global Zustand stores (Auth, User Settings)
...
Server State (API): Use TanStack Query (useQuery, useMutation) for all server-side data (e.g., TUS questions, user profile). Do not store API data in global client stores.

Global Client State: Use Zustand for lightweight global UI state (e.g., theme toggle, simple auth tokens). Avoid Redux Toolkit unless the state complexity is extreme.

Local State: Use standard useState for state limited to a single component.
...
Expo Router: Always prefer file-based routing over manual stack definitions.

Linking: Use the Link component from expo-router or the router object from useRouter for programmatic navigation.

Type Safety: Utilize Expo Router's statically typed routes. Avoid hardcoded strings for complex paths when possible.
...
Use expo-image for high-performance image loading, blurring, and caching.
