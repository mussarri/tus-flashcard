...
interface TopicUsage {
  ...
  records: TopicUsageRecord[];
}

export default function ByTopicPage() {
  const [topicId, setTopicId] = useState("");
  const [usage, setUsage] = useState<TopicUsage | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await api.getAIUsageByTopic(topicId.trim());
      if (response.success) {
        setUsage(response.data);
      }
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to load usage for topic"
      );
      setUsage(null);
    } finally {
      setLoading(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            View token usage and costs for a specific topic
          </p>
        </div>
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Enter topic name (e.g., Diyabet, Epilepsi)"
            ...
            {usage.records.length === 0 ? (
              <div className="bg-white rounded-lg shadow-md p-12 text-center">
                <p className="text-gray-600">
                  No usage records found for this topic
                </p>
              </div>
            ) : (
        ...
        {!usage && !error && (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-600">
              Enter a topic ID to view usage analytics
            </p>
          </div>
        )}
      </div>
